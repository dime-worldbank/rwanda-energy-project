...
epc_join <- rbind(karongi_epc, rulindo_epc, rutsiro_epc) %>% 
  mutate(
    key = paste0(first_name, last_name, nid),
    nid_epc = nid
  ) %>% 
  select(key, readyboard, nid_epc) %>% 
  left_join(complete_krr, by = c("key" = "key")) %>% 
  mutate(
...
rusizi_dime <- read_xlsx(path = file.path(data_path_2, "EDCL", "Readyboard by lot", "Lot_Rusizi-2.xlsx"),
                          sheet = "household list")
rusizi_dime_village <- read_xlsx(path = file.path(data_path_2, "EDCL", "Readyboard by lot", "Lot_Rusizi-2.xlsx"),
                                  sheet = "village list")
...
rusizi_epc <- read_xlsx(path = file.path(data_path_2, "Beneficiary List of Rusizi_2.xlsx"), sheet = "Within scope")
rusizi_epc_noscope <- read_xlsx(path = file.path(data_path_2, "Beneficiary List of Rusizi_2.xlsx"), sheet = "Outside of scope")


rusizi_epc <- rusizi_epc %>% 
  mutate(readyboard = 1)

rusizi_epc_noscope <- rusizi_epc_noscope %>% 
  mutate(readyboard = 0)


rusizi_epc <- rbind(rusizi_epc, rusizi_epc_noscope)


check <- rusizi_epc %>% 
  filter(! nid %in% rusizi_dime$nid)
