...
ggsave(file.path(output_path, "figures", "plot1_appliance.jpeg"), 
       plot = appliance, 
       device = "jpeg", 
       width = 16, 
       height = 8, 
...
ggsave(file.path(output_path, "figures", "plot2_wtp_12.jpeg"), 
       plot = wtp_12, 
       device = "jpeg", 
       width = 16, 
       height = 8, 
...
ggsave(file.path(output_path, "figures", "plot3_lightbulb.jpeg"), 
       plot = lightbulb, 
       device = "jpeg", 
       width = 16, 
       height = 8, 
...
# Save the plot
ggsave(
  filename = "wtp_vs_demand.png",          # output file name
  plot = plot_scatter,                         # the plot object
  path = file.path(output_path, "figures"),                  # folder path
  width = 8, height = 6, dpi = 300         # size and resolution
)
# 
# ggplot(
#   wtp_regress %>% 
...
#   ggsave(file.path(output_path, "figures", filename),
#          plot = p, device = "jpeg", 
#          width = 16, 
#          height = 8, 
#          dpi = 300,
...
for (v in var_list) {
  ...
  ggsave(
    file.path(output_path, "figures", filename),
    plot = p, device = "jpeg",
