...
rusizi_na_distinct<- rusizi_customer %>% 
  filter(is.na(village_id)) %>% 
  distinct(Village, Cell, Sector)

# write_xlsx(rusizi_na, path = file.path(data_path_2, "rusizi customer village not found.xlsx"))

rusizi_na_edited <- read_xlsx(path = file.path(data_path_2, "rusizi customer village not found.xlsx")) 



rusizi_na_edited <- left_join(rusizi_na_edited,rwa_village ,  
                              by =  c("Village" = "village", "Cell" = "cell", "Sector" = "sector", "district" = "district")) %>%
  select(village_id.y, Customer_Name) %>% 
  rename(village_id = village_id.y)

rusizi_customer.1 <- rusizi_customer %>% 
  filter(!is.na(village_id)) %>% select(village_id, Customer_Name) 


rusizi_customer <- rbind(rusizi_customer.1 , rusizi_na_edited)


rusizi_check <- rusizi_dime %>% 
  filter(village_id %in% rusizi_customer$village_id)
  
rusizi1_customer<- rusizi1_customer %>% 
  clean_names() %>% 
...
rusizi1_join <- master %>% 
  filter(district == "rusizi1") 

##first round fuzzy join-----
library(fuzzyjoin)

rusizi1_join.2 <- stringdist_left_join(
  rusizi1_join,
  rusizi1_customer,
  by = c("first_name" = "first_name", "last_name" = "last_name", "villageid_key" = "village_id"),
  method = "jw",
...
# Step 2: Identify unmatched cases
unmatched.1 <- rusizi1_join %>%
  filter(household_id %in% (rusizi1_join.2 %>%
                              filter(is.na(customer)) %>%
                              pull(household_id)))
...
rusizi1_join.2 <- bind_rows(rusizi1_join.2, last_name_village_fuzzy_first) %>%
  arrange(desc(customer)) %>%  # Ensure customer == 1 comes first
  group_by(household_id) %>%
  slice(1) %>%
  ungroup()
...
rusizi1_join.2 <- bind_rows(rusizi1_join.2, first_name_village_fuzzy_last) %>%
  arrange(desc(customer)) %>%  # Ensure customer == 1 comes first
  group_by(household_id) %>%
  slice(1) %>%
  ungroup()
...
#   )
# 
# write_xlsx(still_unmatched_list, path = file.path(data_path_2, "rusizi1 readyboard customer mismatch.xlsx") 

rusizi1_readyboard <- rusizi1_epc %>% 
  filter(readyboard == 1)

rusizi1_customer_analysis <- rusizi1_join.2 %>% 
  group_by(villageid_key) %>%
  summarise(
    n_vulnerable = n(),
    n_readyboard = sum(readyboard == 1, na.rm = TRUE),
  ...
  filter(
    !villageid_key %in% rusizi1_readyboard$villageid_key
  ) %>% 
  bind_rows(
    summarise(.,
              villageid_key = "Total",
