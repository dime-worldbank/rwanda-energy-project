...
bal_hh <- make_balance_table(df = hfc_bal_df, 
                             vars = desc_stats_variables, 
                             cluster = TRUE, 
                             cluster_var = "village")
...
# Build balance table
balance_results <- map_dfr(desc_stats_variables, function(var){
  
  df_var <- hfc_bal_df %>%
    select(village, treatment, all_of(var)) %>%
    filter(!is.na(.data[[var]]))
  
  if (nrow(df_var) == 0L) {
    return(tibble(
      variable = var,
      `mean_C(sd)` = NA_character_,
      `mean_T1(sd)` = NA_character_,
      `mean_T2(sd)` = NA_character_,
      `mean_T3(sd)` = NA_character_,
      `diff_T1–C (se)` = NA_character_,
      `diff_T2–C (se)` = NA_character_,
      `diff_T3–C (se)` = NA_character_,
      p_T1 = NA_real_, p_T2 = NA_real_, p_T3 = NA_real_,
      N_C = NA_integer_, N_T1 = NA_integer_, N_T2 = NA_integer_, N_T3 = NA_integer_,
      clusters_C = NA_integer_, clusters_T1 = NA_integer_, clusters_T2 = NA_integer_, clusters_T3 = NA_integer_,
      clusters_total = NA_integer_
    ))
  }
  ...
  stats <- df_var %>%
    ...
    select(treatment, mean_sd, n) %>%
    pivot_wider(names_from = treatment, values_from = c(mean_sd, n), names_sep = "_")
  
  # Cluster counts by arm
  clusters_by_arm <- df_var %>%
    group_by(treatment) %>%
    summarise(clusters = n_distinct(village), .groups = "drop") %>%
    pivot_wider(names_from = treatment, values_from = clusters,
                names_prefix = "clusters_", values_fill = 0)
  
  clusters_total <- df_var %>% pull(village) %>% n_distinct()
  
  # Regress var on treatment (C is base), CR2 SEs clustered by village
  fml <- as.formula(paste0(var, " ~ treatment"))
  fit <- lm(fml, data = df_var)
  V <- vcovCR(fit, cluster = df_var$village, type = "CR2")
  
  est <- tidy(fit, vcov = V) %>%
    filter(term %in% c("treatmentT1","treatmentT2","treatmentT3")) %>%
    mutate(
      arm = str_replace(term, "treatment", ""),
      diff_se_star = sprintf("%.2f (%.2f)%s", estimate, std.error, starify(p.value))
    ) %>%
    select(arm, diff_se_star, p.value) %>%
    pivot_wider(names_from = arm, values_from = c(diff_se_star, p.value))
  
  tibble(
    variable = var,
    `mean_C(sd)`  = stats[["mean_sd_C"]]  %||% NA_character_,
    `mean_T1(sd)` = stats[["mean_sd_T1"]] %||% NA_character_,
    `mean_T2(sd)` = stats[["mean_sd_T2"]] %||% NA_character_,
    `mean_T3(sd)` = stats[["mean_sd_T3"]] %||% NA_character_,
    
    `diff_T1–C (se)` = est[["diff_se_star_T1"]] %||% NA_character_,
    `diff_T2–C (se)` = est[["diff_se_star_T2"]] %||% NA_character_,
    `diff_T3–C (se)` = est[["diff_se_star_T3"]] %||% NA_character_,
    
    p_T1 = est[["p.value_T1"]] %||% NA_real_,
    p_T2 = est[["p.value_T2"]] %||% NA_real_,
    p_T3 = est[["p.value_T3"]] %||% NA_real_,
    
    N_C  = stats[["n_C"]]  %||% NA_integer_,
    N_T1 = stats[["n_T1"]] %||% NA_integer_,
    N_T2 = stats[["n_T2"]] %||% NA_integer_,
    N_T3 = stats[["n_T3"]] %||% NA_integer_,
    
    clusters_C  = clusters_by_arm[["clusters_C"]]  %||% NA_integer_,
    clusters_T1 = clusters_by_arm[["clusters_T1"]] %||% NA_integer_,
    clusters_T2 = clusters_by_arm[["clusters_T2"]] %||% NA_integer_,
    clusters_T3 = clusters_by_arm[["clusters_T3"]] %||% NA_integer_,
...
# 4) Build balance table (no clustering; HC2 robust SEs)
balance_results.2 <- map_dfr(desc_stats_variables.2, function(var){
  
  df_var <- vill_bal_df %>%
    select(villageid_key, treatment, all_of(var)) %>%
    filter(!is.na(.data[[var]]))
  ...
  # Regression with C as base and HC2 robust SE
  fml <- as.formula(paste0(var, " ~ treatment"))
  fit <- lm(fml, data = df_var)
  V   <- sandwich::vcovHC(fit, type = "HC2")
  
  est <- broom::tidy(fit, vcov = V) %>%
    dplyr::filter(term %in% c("treatmentT1","treatmentT2","treatmentT3")) %>%
    mutate(
      arm = str_replace(term, "treatment", ""),
      diff_se_star = sprintf("%.2f (%.2f)%s", estimate, std.error, starify(p.value))
    ) %>%
