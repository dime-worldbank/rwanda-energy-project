...
village_181_save <- complete_status %>% 
  filter(
    `Dropped from scope due to 15kv` == "No"
  ) %>% 
  distinct(villageid_key, .keep_all = TRUE) %>% 
...
write_xlsx(village_181_save, path = file.path(output_path, "Surveyed 181 villages.xlsx"))
...
hfc_date <- hfc_constr_raw %>%
  select(hh_id, submissiondate) %>%
  group_by(hh_id) %>%
  slice_max(order_by = submissiondate, n = 1, with_ties = FALSE) %>%
  ungroup() %>% 
  rename(`Baseline survey submitted date` = submissiondate)


hfc_screening_date <- screening_raw %>% 
  select(hh_id, endtime) %>%
  group_by(hh_id) %>%
  slice_max(order_by = endtime, n = 1, with_ties = FALSE) %>%
  ungroup() %>% 
  rename(`Screening survey submitted date` = endtime) %>% 
  mutate(hh_id = as.character(hh_id))



complete_save_join <- left_join(complete_save_join, hfc_date, by = c("household_id" = "hh_id")) %>%
  left_join(hfc_screening_date, , by = c("household_id" = "hh_id"))
