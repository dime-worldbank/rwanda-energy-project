...
run_reg_est <- function(df) {
  list(
    ...
    num_est1 = felm(num_establishment ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                    | village_id + cell_year | 0 | sector_id, data = df),
    
    num_est2 = felm(num_establishment ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                    | village_id + cell_year + cell_office_year | 0 | sector_id, data = df),
    
    num_est3 = felm(num_establishment ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                    | village_id + cell_year + cell_office_year + health_center_year | 0 | sector_id, data = df),
    
    num_est4 = felm(num_establishment ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                    | village_id + cell_year + cell_office_year + health_center_year + primary_school_year | 0 | sector_id, data = df),
    
    num_est5 = felm(num_establishment ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                    | village_id + cell_year + cell_office_year + health_center_year + primary_school_year + secondary_school_year | 0 | sector_id, data = df),
    
    num_est6 = felm(num_establishment ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
    ...
    num_est7 = felm(num_establishment ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
    ...
    num_est8 = felm(num_establishment ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
...
run_reg_emp <- function(df) {
  list(
    ...
    emp1 = felm(total_employee ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                | village_id + cell_year | 0 | sector_id, data = df),
    
    emp2 = felm(total_employee ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                | village_id + cell_year + cell_office_year | 0 | sector_id, data = df),
    
    emp3 = felm(total_employee ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                | village_id + cell_year + cell_office_year + health_center_year | 0 | sector_id, data = df),
    
    emp4 = felm(total_employee ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                | village_id + cell_year + cell_office_year + health_center_year + primary_school_year | 0 | sector_id, data = df),
    
    emp5 = felm(total_employee ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                | village_id + cell_year + cell_office_year + health_center_year + primary_school_year + secondary_school_year | 0 | sector_id, data = df),
    
    emp6 = felm(total_employee ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                | village_id + cell_year + cell_office_year + health_center_year + primary_school_year + secondary_school_year + sector_district_office_year | 0 | sector_id, data = df),
    
    emp7 = felm(total_employee ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                | village_id + cell_year + cell_office_year + health_center_year + primary_school_year + secondary_school_year + sector_district_office_year + industry_year | 0 | sector_id, data = df),
    
    emp8 = felm(total_employee ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                | village_id + cell_year + cell_office_year + health_center_year + primary_school_year + secondary_school_year + sector_district_office_year + industry_year + market_year | 0 | sector_id, data = df)
  )
}
...
regs_isic16_est <- run_reg_est(earp_did_16)
regs_isic16_emp <- run_reg_emp(earp_did_16)

# Export temporary stargazer outputs for extraction
tex_file_est <- file.path(output_path, "regressions", "raw", "EARP_ISIC16_est_raw.tex")
tex_file_emp <- file.path(output_path, "regressions", "raw", "EARP_ISIC16_emp_raw.tex")
...
mean_line <- paste(
  "Mean &",
  sprintf("%.3f", compute_mean(earp_did_16, "num_establishment")), 
  "& \\multicolumn{7}{c}{} \\\\"
)
...
writeLines(c(
  ...
  "}%",
  "\\end{table}"
), file.path(output_path,"regressions", "EARP_ISIC16_est.tex"))
...
mean_line <- paste(
  "Mean &",
  sprintf("%.3f", compute_mean(earp_did_16, "total_employee")), 
  "& \\multicolumn{7}{c}{} \\\\"
)
writeLines(c(
  "\\begin{table}[!htbp]",
  ...
  "}%",
  "\\end{table}"
), file.path(output_path,"regressions", "EARP_ISIC16_emp.tex"))
...
regs_private_est <- run_reg_est(earp_p)
regs_private_emp <- run_reg_emp(earp_p)

# Export raw stargazer files
tex_file_priv_est <- file.path(output_path,"regressions", "raw", "EARP_Private_est_raw.tex")
tex_file_priv_emp <- file.path(output_path, "regressions", "raw", "EARP_Private_emp_raw.tex")
...
mean_line <- paste(
  "Mean &",
  sprintf("%.3f", compute_mean(earp_p, "num_establishment")), 
  "& \\multicolumn{7}{c}{} \\\\"
)
writeLines(c(
  "\\begin{table}[!htbp]",
  ...
  "}%",
  "\\end{table}"
), file.path(output_path,"regressions", "EARP_Private_est.tex"))



mean_line <- paste(
  "Mean &",
  sprintf("%.3f", compute_mean(earp_p, "total_employee")), 
  "& \\multicolumn{7}{c}{} \\\\"
)
...
writeLines(c(
  ...
  "}%",
  "\\end{table}"
), file.path(output_path,"regressions", "EARP_Private_emp.tex"))
...
regs_public_est <- run_reg_est(earp_pu)
regs_public_emp <- run_reg_emp(earp_pu)

# Export raw stargazer
tex_file_pub_est <- file.path(output_path,  "regressions", "raw", "EARP_Public_est_raw.tex")
tex_file_pub_emp <- file.path(output_path,  "regressions", "raw", "EARP_Public_emp_raw.tex")
...
mean_line <- paste(
  "Mean &",
  sprintf("%.3f", compute_mean(earp_pu, "num_establishment")), 
  "& \\multicolumn{7}{c}{} \\\\"
)
# Final Public tables
writeLines(c(
  ...
  "}%",
  "\\end{table}"
), file.path(output_path,"regressions", "EARP_Public_est.tex"))
...
mean_line <- paste(
  "Mean &",
  sprintf("%.3f", compute_mean(earp_pu, "total_employee")), 
  "& \\multicolumn{7}{c}{} \\\\"
)
...
writeLines(c(
  ...
  "}%",
  "\\end{table}"
), file.path(output_path, "regressions","EARP_Public_emp.tex"))
...
private_priority_est <- felm(num_establishment ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                | village_id + cell_year + cell_office_year + health_center_year + 
                  ...
                  industry_year + market_year + priority_year | 0 | sector_id, data = earp_p)

summary(private_priority_est)

private_priority_emp <- felm(total_employee ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                             | village_id + cell_year + cell_office_year + health_center_year + 
                               primary_school_year + secondary_school_year + sector_district_office_year + 
                               industry_year + market_year + priority_year | 0 | sector_id, data = earp_p)
...
public_priority_est<- felm(num_establishment ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                         | village_id + cell_year + cell_office_year + health_center_year + 
                           primary_school_year + secondary_school_year + sector_district_office_year + 
                           industry_year + market_year + priority_year | 0 | sector_id, data = earp_pu)

summary(public_priority_est)



public_priority_emp<- felm(total_employee ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`
                           | village_id + cell_year + cell_office_year + health_center_year + 
                             primary_school_year + secondary_school_year + sector_district_office_year + 
                             industry_year + market_year + priority_year | 0 | sector_id, data = earp_pu)
...
stargazer(
  models,
  type = "latex",
  out = file.path(output_path, "regressions", "earp_priority.tex"),
  title = "Regression Results: EARP with Priority Fixed Effects",
  ...
  covariate.labels = c("EARP × 2014", "EARP × 2017", "EARP × 2020"),
  keep = c("p0_2014:EARP", "EARP:p1_2017", "EARP:p2_2020"),
  omit.stat = c("adj.rsq", "ser", "f", "ll", "aic", "bic"),
  keep.stat = c("n", "rsq"),
  float = TRUE,
