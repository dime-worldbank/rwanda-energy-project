#######################################
#Purpose: elec12_14 as a control for analysis
...
balance_cell_elec12_14 <- balance_table(
  data = elec12_14_p %>% filter(year == 2011),
  treatment = c("elec12_14"),
  cluster = sector_id,
  fe = c("cell_id")
)

balance_consumer_elec12_14 <- balance_table(
  data = elec12_14_p %>% filter(year == 2011),
  treatment = c("elec12_14", "log1_residential_consumer", "log1_non_residential_consumer"),
  cluster = sector_id,
...
balance_cell_elec12_14.3 <- balance_table(
  data = elec12_14_p.3 %>% filter(year == 2011),
  treatment = c("elec12_14"),
  cluster = sector_id,
  fe = c("cell_id")
)

balance_consumer_elec12_14.3 <- balance_table(
  data = elec12_14_p.3 %>% filter(year == 2011),
  treatment = c("elec12_14", "log1_residential_consumer", "log1_non_residential_consumer"),
  cluster = sector_id,
  fe = c("cell_id", "cell_office", "health_center",
...
balance_cell_elec12_14.7 <- balance_table(
  data = elec12_14_p.7 %>% filter(year == 2011),
  treatment = c("elec12_14"),
  cluster = sector_id,
  fe = c("cell_id")
)

balance_consumer_elec12_14.7 <- balance_table(
  data = elec12_14_p.7 %>% filter(year == 2011),
  treatment = c("elec12_14", "log1_residential_consumer", "log1_non_residential_consumer"),
  cluster = sector_id,
  fe = c("cell_id", "cell_office", "health_center",
...
balance_cell_elec12_14.9 <- balance_table(
  data = elec12_14_p.9 %>% filter(year == 2011),
  treatment = c("elec12_14"),
  cluster = sector_id,
  fe = c("cell_id")
)

balance_consumer_elec12_14.9 <- balance_table(
  data = elec12_14_p.9 %>% filter(year == 2011),
  treatment = c("elec12_14", "log1_residential_consumer", "log1_non_residential_consumer"),
  cluster = sector_id,
  fe = c("cell_id", "cell_office", "health_center",
...
balance_cell_elec12_14.19 <- balance_table(
  data = elec12_14_p.19 %>% filter(year == 2011),
  treatment = c("elec12_14"),
  cluster = sector_id,
  fe = c("cell_id")
)

balance_consumer_elec12_14.19 <- balance_table(
  data = elec12_14_p.19 %>% filter(year == 2011),
  treatment = c("elec12_14", "log1_residential_consumer", "log1_non_residential_consumer"),
  cluster = sector_id,
  fe = c("cell_id", "cell_office", "health_center",
...
balance_cell_all <- bind_rows(
  balance_cell_elec12_14      %>% mutate(sector = "All sectors"),
  balance_cell_elec12_14.3    %>% mutate(sector = "Manufacture (.3)"),
  balance_cell_elec12_14.7    %>% mutate(sector = "Wholesale (.7)"),
  balance_cell_elec12_14.9    %>% mutate(sector = "Food and Accomm (.9)"),
  balance_cell_elec12_14.19   %>% mutate(sector = "Other Services (.19)")
)


# 2️⃣ Combine all "cell + targeting FE" balance tables
balance_consumer_all <- bind_rows(
  balance_consumer_elec12_14      %>% mutate(sector = "All sectors"),
  balance_consumer_elec12_14.3    %>% mutate(sector = "Manufacture (.3)"),
  balance_consumer_elec12_14.7    %>% mutate(sector = "Wholesale (.7)"),
  balance_consumer_elec12_14.9    %>% mutate(sector = "Food and Accomm (.9)"),
  balance_consumer_elec12_14.19   %>% mutate(sector = "Other Services (.19)")
)
...
writeLines(latex_lines_final, file.path(output_path, "balance table (2011).tex"))
...
balance_infra_elec12_14 <- balance_table(
  data = elec12_14_p %>% filter(year == 2011),
  treatment = c("elec12_14"),
  cluster = sector_id,
  fe = c("cell_id", "cell_office", "health_center",
...
balance_imidugudu_elec12_14 <- balance_table(
  data = elec12_14_p %>% filter(year == 2011),
  treatment = c("elec12_14"),
  cluster = sector_id,
  fe = c("cell_id", "cell_office", "health_center",
...
# === Combine and Export to LaTeX (elec12_14) ===


combined_diff <- balance_cell_elec12_14 %>%
  select(outcome, N_control, N_treated, control, treated, diff_cell = diff) %>%
  left_join(balance_infra_elec12_14 %>%
              select(outcome, diff_infra = diff), by = "outcome") %>%
  left_join(balance_imidugudu_elec12_14 %>%
              select(outcome, diff_imidugudu = diff), by = "outcome") %>%
  left_join(balance_consumer_elec12_14 %>%
...
# Create LaTeX table
kable_output_elec12_14 <- combined_diff %>%
  kable(
    format = "latex",
    booktabs = TRUE,
    align = "lcccccc",
    caption = "Electrified 2012–2014 on 2011 EC Outcomes (Balance Table)",
    label = "tab:balance_elec12_14_fe"
  ) %>%
  kable_styling(
...
# Save LaTeX to file
writeLines(
  as.character(kable_output_elec12_14),
  file.path(output_path, "balance_table", "balance_table_elec12_14_p.tex")
)
...
# #ImBalance table--------
# 
# elec12_14_median <- elec12_14_did_all %>% 
#   filter(elec12_14 == 1) %>% 
#   left_join(utility_long_2020)
# 
# elec12_14_did_median <- elec12_14_median %>% 
#   mutate(usage_2020_median = if_else(usage_2020 > median(elec12_14_median$usage_2020), 1, 0)) 
# 
# 
# balance_none_elec12_14 <- balance_table(
#   data = elec12_14_did_median %>% filter(year == 2011),
#   treatment = usage_2020_median,
#   cluster = cell_id
# )
# 
# balance_cellid_elec12_14 <- balance_table(
#   data = elec12_14_did_median %>% filter(year == 2011),
#   treatment = usage_2020_median,
#   cluster = cell_id,
#   fe = c("cell_id")
# )
# 
# balance_allfe_elec12_14 <- balance_table(
#   data = elec12_14_did_median %>% filter(year == 2011),
#   treatment = usage_2020_median,
#   cluster = cell_id,
#   fe = c("cell_id", "cell_office", "health_center",
...
# # === Combine and Export to LaTeX (elec12_14) ===
# 
# combined_diff_elec12_14 <- balance_none_elec12_14 %>%
#   select(outcome, N_control, N_treated,control, treated, diff_none = diff) %>%
#   left_join(balance_cellid_elec12_14 %>% select(outcome, diff_cellid = diff), by = "outcome") %>%
#   left_join(balance_allfe_elec12_14 %>% select(outcome, diff_allfe = diff), by = "outcome") %>%
#   rename(
#     `Control (Mean (SD))` = control,
#     `Treated (Mean (SD))` = treated,
...
#   )
# 
# kable_output_elec12_14 <- combined_diff_elec12_14 %>%
#   kable(
#     format = "latex",
#     booktabs = TRUE,
#     align = "lccccc",
...
# 
# # Save to file
# writeLines(as.character(kable_output_elec12_14),
#            file.path(output_path, "balance_table", "balance_table_elec12_14_usage.tex"))
# 
...
kable_output <- combined_diff_p %>%
  kable(
    ...
    caption = "elec12-14 on 2011 establishment census private sector output",
...
balance_none_pu <- balance_table(
  data = elec12_14_pu %>% filter(year == 2011),
  treatment = c("elec12_14"),
  cluster = cell_id
)
...
kable_output <- combined_diff_pu %>%
  kable(
    ...
    booktabs = TRUE,
    align = "lcccccc", # adjust if you add/remove columns
    caption = "elec12-14 on 2011 establishment census public sector output",
    label = "tab:balance_fe_compare"
  ) %>%
  kable_styling(latex_options = c("hold_position", "scale_down"), full_width = FALSE)
...
# Save to file
writeLines(kable_output,
           file.path(output_path, "balance_table", "balance_table_elec12_14_pu.tex"))
...
kable_output <- combined_all_private_public %>%
  kable(
    ...
    booktabs = TRUE,
    align = "lcccccc",
    caption = "Elec12-14 on 2011 establishment census",
...
writeLines(
  kable_output,
  file.path(output_path, "balance_table", "balance_table_elec12_14_all.tex")
)
