...
earp <- expansion_join_drop13%>%
  select(village_id, earp_mv, earp_lv, cell_office, health_center, primary_school) %>%
  rename(
    mv = earp_mv,
    lv = earp_lv
  )
...
num_establishment <- felm(num_establishment ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`|village_id + cell_year + cell_office_year + health_center_year + primary_school_year |0|sector_id, data = earp_did)
summary(num_establishment)

total_employee <- felm (total_employee ~ p0_2014*`EARP` + p1_2017*`EARP` + p2_2020*`EARP`|village_id + cell_year + cell_office_year + health_center_year + primary_school_year|0|sector_id, data = earp_did)
