...
lightbulb <- hfc_constr %>% 
  filter(
    hh_id %in% elec_check$hh_id
  ) %>% 
  select(
    hh_id, village, village_key, enumerator, enumerator_key, submissiondate, first_name, last_name, phonenumber, second_phonenumber,
    A1_2, H1_1, A1_3, H2_1, H8_6, C3_14
  )


hfc_sheet %>% 
  sheet_write(data = lightbulb, sheet = "lightbulb_check")
...
#Order of the replacement sheet----
