...
four_scope<- read_xlsx(path = file.path(data_path, "scope_193_0807.xlsx"))
...
energy_counts <- hfc_constr %>%
  ...
  summarise(
    count = n(),
    percentage = paste0(round(count / nrow(.)* 100, 2), "%")
  ) %>% 
  left_join(energy_source_choices, by = c("H3_1" = "code"))
