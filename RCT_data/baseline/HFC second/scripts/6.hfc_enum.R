...
enumerator_check <- left_join(enumerator_check, mean_surveys_per_enumerator_per_day, by = c("enumerator")) %>% 
  select(-enumerator_key)

enumerators <- enumerators %>% 
  filter(!is.na(enumeratorid_key)) %>% 
  mutate(
    enumeratorid_key = as.character(enumeratorid_key),
  ) %>% 
  select(
    enumeratorid_key, enumerator_key
  )

enumerator_check_final <- left_join(enumerators, enumerator_check, by = c("enumeratorid_key" = "enumerator"))

hfc_sheet %>%
  sheet_write(data = enumerator_check_final, sheet = "enumerator_check")
