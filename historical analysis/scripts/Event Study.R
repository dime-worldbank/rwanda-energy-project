...
pacman::p_load(raster,exactextractr,dplyr, rgdal,here,ggplot2,sf,tidyr,readxl,writexl, stringr, stargazer, janitor)
...
coefs_long11 <- coefs_long %>% 
  filter(group == "connect_2011")



ggplot(coefs_long11, aes(x = as.numeric(year), y = value, group = group, color = group)) +
  geom_line() +
  geom_point() +
