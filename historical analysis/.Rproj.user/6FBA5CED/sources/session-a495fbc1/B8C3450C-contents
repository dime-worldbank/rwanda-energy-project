...
bar_plot <- rwa_regress %>% 
 filter(!district %in% c("Ngororero", "Nyabihu", "Nyamasheke", "Rubavu") ) %>% 
  mutate(
    status = case_when(
      year_first <= 2011 ~ "always_electrified",
      year_first <= 2014 & year_first >= 2012 ~ "2012-2014",
      year_first <= 2017 & year_first >= 2015 ~ "2015-2017",
      year_first <= 2020 & year_first >= 2018 ~ "2018-2020",
      year_first <= 2022 & year_first >= 2021 ~ "2021-2022",
      year_first == 2300 | is.na(year_first) ~ "never_electrified"
    )
  ) %>% 
...
bar_plot$status <- factor(bar_plot$status, levels = c("always_electrified", "2012-2014", "2015-2017", "2018-2020", "2021-2022", "never_electrified"))
...
ggplot(bar_plot, aes(x = status, y = n)) +
  ...
  labs(x = "Year of Electrification", y = "Number of sector", title = "Number of Villages by Electrification Year") +
  theme_minimal() +
  theme(
...
rwa_cell$status <- factor(rwa_cell$status, levels = c("always_elec", "2012-2014", "2015-2017", "2018-2020", "2021-2022", "never_elec"))
